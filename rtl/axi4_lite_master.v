module axi4_lite_master #(
   ...
   // output reg                        wr_done,
    output reg  [1:0]                 bresp_out,  // Captured BRESP
    ...
    input  wire [2:0]                 rd_prot,  // Per transaction ARPROT
                                                // Tied to DEFAULT_PROT
 //   output reg                        rd_done,
    output reg  [DATA_WIDTH-1:0]      data_out,  // Captured RDATA
    output reg  [1:0]                 rresp_out   // Captured RRESP
);
        ...
        case (wr_state)
            ...
            W_RESP: begin
                if (BVALID && BREADY) begin
                    bresp_out <= BRESP;   // Captured: OKAY, SLVERR, or DECERR
               //     wr_done <= 1'b1;
                    BREADY  <= 1'b0;
                end
            end
        ...
        case (rd_state)
            ...
            if (ARREADY)
                ...
                if (RVALID && RREADY) begin
                    data_out <= RDATA;   
                    rresp_out <= RRESP;   // Captured: OKAY, SLVERR, or DECERR
                //    rd_done <= 1'b1;
                    RREADY  <= 1'b0;
                end
            end
